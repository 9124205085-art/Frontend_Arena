...
import type { UniversePayload } from "./universePayload";
import type { Receipt } from "../data/types";

const CAMERA = { position: [0, 0.32, 10.8] as [number, number, number], fov: 40, near: 0.1, far: 48 };
const DPR_M: [number, number] = [1, 1];
const DPR_D: [number, number] = [1, 1.35];
const GL_M = { antialias: false, alpha: true, powerPreference: "high-performance" as const, stencil: false, depth: true };
const GL_D = { antialias: true, alpha: true, powerPreference: "high-performance" as const, stencil: false, depth: true };
...
const TORUS: [number, number, number, number] = [1.42, 0.006, 6, 64];
const TORUS_2: [number, number, number, number] = [2.05, 0.004, 6, 48];
const GROUND_ROT: [number, number, number] = [-Math.PI / 2, 0, 0];
const GROUND_POS: [number, number, number] = [0, -2.55, 0];
const GROUND_ARGS: [number, number] = [4.8, 32];
...
function disposeObject(object: THREE.Object3D | THREE.BufferGeometry | THREE.Material | null | undefined) {
  if (!object) return;
...
export default function MemoryUniverse({
  ...
  useEffect(() => {
    ...
    const sync = () => {
      ...
      setPaused(hidden || !node.isConnected);
    };

    const onVis = () => {
      setPaused(document.visibilityState !== "visible" || !isIntersecting.current);
    };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Core />
        <GroundGlow />
        <Fragments sample={payload.sample} mouse={mouse} />
        {!mobile && <Threads sample={payload.sample} pairs={payload.threadPairs} />}
        <Dust count={mobile ? 36 : 88} />
...
function GroundGlow() {
  ...
  useEffect(() => () => disposeObject(geo), [geo]);
  return (
...
function Core() {
  ...
  const meshes = useMemo(() => {
    ...
    const glow = new THREE.SphereGeometry(0.92, 20, 20);
  ...
  useEffect(
    () => () => {
      disposeObject(meshes.core);
      disposeObject(meshes.glow);
      disposeObject(meshes.ring);
      disposeObject(meshes.ring2);
    },
    [meshes],
  );
...
function Fragments({
  ...
  useEffect(
    () => () => {
      pack.inst.geometry.dispose();
      (pack.inst.material as THREE.Material).dispose();
      pack.inst.dispose();
    },
    [pack],
  );
...
function Threads({ sample, pairs }: { sample: Receipt[]; pairs: Array<[string, string]> }) {
  const line = useMemo(() => {
  ...
  useEffect(() => () => disposeObject(line ?? undefined), [line]);
...
function Dust({ count }: { count: number }) {
  ...
  useEffect(() => () => disposeObject(points), [points]);
